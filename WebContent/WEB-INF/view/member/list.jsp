  ...
  <table class="table">
    ...
    <tbody>
       <c:forEach var="i" items="${requestScope.list}">
		      <tr class="success">
		      	<td>${i.num}</td>
		      	<td> <a onclick="detailStudent('${i.member_id}')">${i.member_id}</a></td>
		      	 <td>${i.name}</td>
		      	 <td>${i.regdate}</td>
		      	 <td>${i.phone}</td>
		      	 <td>${i.email}</td>
...
function pageClick(pageNumber){
	location.href="${ctx}/member.do?action=list&page=list&pageNumber="+pageNumber;
}
function next(){
	var nextPage=${requestScope.startPage}+${requestScope.blockSize};
	location.href="${ctx}/member.do?action=list&page=list&pageNumber="+nextPage;
}
function prev(){
	...
	location.href="${ctx}/member.do?action=list&page=list&pageNumber="+prevPage;
}
function nextnext(){
	var nextnextPage=${requestScope.theNumberOfPages};
	location.href="${ctx}/member.do?action=list&page=list&pageNumber="+nextnextPage;
}
function prevprev(){
	var prevPage=${requestScope.endPage}-${requestScope.blockSize};
	location.href="${ctx}/member.do?action=list&page=list&pageNumber=1";
}
